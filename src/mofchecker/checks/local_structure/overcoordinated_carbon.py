...
from ..utils.get_indices import _is_any_neighbor_metal, get_c_indices,_is_any_neighbor_boron
...
class OverCoordinatedCarbonCheck(BaseCoordinationCheck):
    ...
    def _get_overcoordinated_carbons(self):
        """Check for all C if CN>4, ignore metal bonds. ignore carborane"""
        ...
        for site_index in self.c_indices:
            ...
            if cn > 4 and not _is_any_neighbor_metal(self.get_connected_sites(site_index)):
                if not _is_any_neighbor_boron(self.get_connected_sites(site_index)):
